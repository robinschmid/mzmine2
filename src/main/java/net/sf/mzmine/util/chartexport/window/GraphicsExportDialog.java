...
import net.sf.mzmine.util.chartexport.SettingsExportGraphics;
...
public class GraphicsExportDialog extends JFrame implements SettingsPanel {
	...
	private SettingsExportGraphics sett = new SettingsExportGraphics();
	 ...
	 * @param chart
	 */
	public static void openInstDialog(JFreeChart chart) {
		createInstance().openDialog(chart);
	} 
	protected void openDialog(JFreeChart chart) {
		inst.chart = chart; 
		setVisible(true);
		//
	...
	private void setLegendVisible(boolean selected) {
		setLegendVisible(chart, selected);
	}
	private void setLegendVisible(JFreeChart chart, boolean selected) {
		...
		}
		{
			...
			pnSettingsLeft.setLayout(new BorderLayout(0, 0));
			{
				...
				pnSettingsLeft.add(scrollPane, BorderLayout.CENTER);
				{
					...
					{
						panel = new Module();
						panel.getLbTitle().setText("Chart");
						grid.add(panel);
						{
							panel_3 = new JPanel();
							FlowLayout flowLayout = (FlowLayout) panel_3.getLayout();
							flowLayout.setAlignment(FlowLayout.LEFT);
							panel.getPnContent().add(panel_3, BorderLayout.CENTER);
							{
								cbLegend = new JCheckBox("legend");
								cbLegend.setHorizontalAlignment(SwingConstants.LEFT);
								cbLegend.setSelected(true);
								cbLegend.addItemListener(new ItemListener() { 
									@Override
									public void itemStateChanged(ItemEvent e) {
										boolean state = ((JCheckBox)e.getSource()).isSelected();
										setLegendVisible(state);
									}
								});
