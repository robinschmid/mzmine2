...
public class MSEcorrGroupWindow extends JFrame implements ComponentListener {
	 ...
	 * @param peakList 
	 */
	public MSEcorrGroupWindow(MZmineProject project, final MSEGroupedPeakList peakList, PKLRowGroupList groups, int index) {
		// data
		...
		ParameterSet peakListTableParameters = MZmineCore.getConfiguration().getModuleParameters(PeakListTableModule.class);
		//
		...
		panel_5.add(btnJumpToRT);
				...
				panel_2.setLayout(new BoxLayout(panel_2, BoxLayout.Y_AXIS));
						...
						cbUseSampleGroups.setToolTipText("Use sample groups or rows as categories for intensity boxplots.");
						...
						cbAutoRawFile.setToolTipText("(Switch raw files with + and - keys) Show peak shapes and correlation data of the raw file with the highest number of peaks.");
	...
	private void addKeyBindings() {
		...
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(commands[0]), commands[0]);
		...
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(commands[1]), commands[1]);
		...
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(commands[2]), commands[2]);
		...
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(commands[3]), commands[3]);
		...
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_ADD, 0), commands[4]);
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_PLUS, 0), commands[4]);
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_EQUALS, KeyEvent.SHIFT_DOWN_MASK), commands[4]);
		...
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_SUBTRACT,0), commands[5]); 
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_MINUS,0), commands[5]); 
		pn.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_EQUALS, KeyEvent.SHIFT_DOWN_MASK), commands[5]);
	...
	/**
	 * check for auto show raw file that contains the lastViewed row 
	 * of the last viewed group
	 */
	...
	/**
	 * renews all plots:
	 * IProfile, peak shapes, peak shape correlation
	 */
	...
	private void plotPeakShapes() {
		...
		if(g!=null) { 
			...
			// add plot
			String sg = String.valueOf(project.getParameterValue(peakList.getSampleGroupsParameter(), raw));
			String title = sg+"("+raw.getName()+")";
			JFreeChart chart = ChartFactory.createXYLineChart(title, "retention time | min", "Intensity", data, PlotOrientation.VERTICAL, true, true, false);
	...
	private void plotPeakShapeCorrelation() {
		...
		if(g!=null) {  
			...
			// create chart
			JFreeChart chart = ChartFactory.createScatterPlot(title, "Intensity (row: "+row.getID()+")", "Intensity (other rows)", data, PlotOrientation.VERTICAL, true, true, false);
			XYItemRenderer renderer = chart.getXYPlot().getRenderer();
			theme.apply(chart);   
			// formating 
			NumberFormat intensityFormat = MZmineCore.getConfiguration().getIntensityFormat();
			...
			if(!getCbShowPseudoSpectrum().isSelected()) {
				...
				for(int r=0; r<peakList.getRawDataFiles().length; r++) {  
					...
					String rawSG = String.valueOf(project.getParameterValue(peakList.getSampleGroupsParameter(), rfile));
					...
					for(int i=0; i<g.size(); i++) {
						...
						if(rowI!=i) { 
							...
							if(fCorr!=null && fCorr.getReg()!=null && fCorr.getData()!=null) {
								...
								else dataset.addValue(reg.getR(), rawSG, rfile.getName()+"("+trow.getID()+")"); 
							}
						}  
					}
				}
				...
				chart = ChartFactory.createBarChart(title, "Sample group", 
	...
	private void plotPseudoSpectrum() {
		PKLRowGroup g = peakList.getLastViewedGroup();
		ChartPanel chart = PseudoSpectrum.createChart(g, g.getLastViewedRawFile(), getCbSumPseudoSpectrum().isSelected());
	...
	/**
	 * plot the height profile of this row and of all rows
	 * in a combined chart
	 */
	private void plotIProfile() {
		...
		if(g!=null) { 
			...
			final DefaultBoxAndWhiskerCategoryDataset datasetAll = new DefaultBoxAndWhiskerCategoryDataset();
	 ...
	 * @return
	 */
	private ChartPanel createCombinedBoxAndWhiskerPlot(BoxAndWhiskerCategoryDataset dataset, BoxAndWhiskerCategoryDataset datasetAll, Paint colorRowI, int rowI) {  
		...
		renderer2.setBaseToolTipGenerator(new BoxAndWhiskerToolTipGenerator());
