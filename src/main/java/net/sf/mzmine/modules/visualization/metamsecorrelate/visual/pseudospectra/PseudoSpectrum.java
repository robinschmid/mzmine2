...
public class PseudoSpectrum {
	...
	public static PseudoSpectrumDataSet createDataSet(PKLRowGroup group, RawDataFile raw, boolean sum) {
		// data
		...
		for(PeakListRow row : group) {
			...
			for(PeakIdentity id : row.getPeakIdentities()) {
				if(id instanceof ESIAdductIdentity) { 
					if(annotation==null) annotation = ((ESIAdductIdentity)id).getAdduct();
					else annotation += "\n" + ((ESIAdductIdentity)id).getAdduct();
				}
			}
			// sum -> heighest peak
			if(sum) series.addDP(row.getAverageMZ(), row.getBestPeak().getHeight(), annotation);
			else {
