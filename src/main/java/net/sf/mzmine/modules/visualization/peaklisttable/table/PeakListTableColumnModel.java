 ...
 * 
 */
public class PeakListTableColumnModel extends DefaultTableColumnModel implements MouseListener {
  ...
  public void createColumns() {
    ...
    for (int i = 0; i < peakList.getNumberOfRawDataFiles(); i++) {
      ...
      for (int j = 0; j < visibleDataFileColumns.length; j++) {
        ...
        int modelIndex = CommonColumnType.values().length + (i * DataFileColumnType.values().length)
            + dataFileColumnIndex;
