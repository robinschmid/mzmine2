...
public class SpectraVisualizerWindow extends JFrame implements ActionListener {
	...
	// mass list number: null for no selection
	private String selectedMassListName = null;
	...
	public void actionPerformed(ActionEvent event) {
		final SpectraVisualizerWindow thiswindow = this;
		...
		if (command.equals("PREVIOUS_SCAN")) {
			...
			if (scanIndex > 0) {
				...
				Runnable newThreadRunnable = new Runnable() {
					...
					public void run() {
						Scan scan = dataFile.getScan(prevScanIndex);
						loadRawData(scan);
		...
		if (command.equals("NEXT_SCAN")) {
			...
			if (scanIndex < (scanNumbers.length - 1)) {
				...
				Runnable newThreadRunnable = new Runnable() {
					...
					public void run() {
						Scan scan = dataFile.getScan(nextScanIndex);
						loadRawData(scan);
	...
	public String getSelectedMassListName() {
		return selectedMassListName;
	}

	public void setSelectedMassListName(String selectedMassList) {
		this.selectedMassListName = selectedMassList;
	}
