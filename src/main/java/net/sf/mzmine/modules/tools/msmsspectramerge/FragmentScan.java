...
class FragmentScan {
  ...
  /**
   * To directly merge an array of scans
   * 
   * @param scans
   * @param massList
   ...
   * @return
   */
  static FragmentScan[] getAllFragmentScansFor(Scan[] scans, String massList,
      Range<Double> isolationWindow, MZTolerance massAccuracy) {
    final List<FragmentScan> fragmentScans = new ArrayList<>();
    final int[] ms2 = Arrays.stream(scans).mapToInt(Scan::getScanNumber).toArray();
    Arrays.sort(ms2);
    for (Scan scan : scans) {
      ...
      while (i < ms2.length) {
        ...
        fragmentScans.add(new FragmentScan(file, null, massList,
            precursorScan != null ? precursorScan.getScanNumber() : null,
            precursorScan2 != null ? precursorScan2.getScanNumber() : null, subms2, isolationWindow,
            massAccuracy));
        i = j;
  ...
  private void detectPrecursor(int ms1Scan, double precursorMass, Range<Double> isolationWindow,
    ...
    // now sum up all remaining intensities. Leave out isotopes. leave out peaks with intensity
    // below 10%
