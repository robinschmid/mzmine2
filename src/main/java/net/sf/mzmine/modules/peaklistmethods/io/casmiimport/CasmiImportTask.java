...
class CasmiImportTask extends AbstractTask {
  ...
  CasmiImportTask(MZmineProject project, ParameterSet parameters) {
    ...
    casmiProblemName = parameters.getParameter(CasmiImportParameters.casmiProblemName).getValue();
    msSpectrum = parameters.getParameter(CasmiImportParameters.msSpectrum).getValue();
    msMsSpectrum = parameters.getParameter(CasmiImportParameters.msMsSpectrum).getValue();
    polarity = parameters.getParameter(CasmiImportParameters.polarity).getValue();
  ...
  public void run() {
    ...
    try {
      ...
      Scan msScan = new SimpleScan(null, msScanNumber, 1, 1.0, 0, 0, new int[] {2},
          msSpectrumDataPoints, MassSpectrumType.CENTROIDED, polarity, "", null);
      ...
      final MassList msScanMassList =
          new SimpleMassList("CASMI", finalMsScan, msSpectrumDataPoints);
      finalMsScan.addMassList(msScanMassList);
      Scan finalMsMsScan = newDataFile.getScan(msMsScanNumber);
      final MassList msMsScanMassList =
          new SimpleMassList("CASMI", finalMsMsScan, msMsSpectrumDataPoints);
