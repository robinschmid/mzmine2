...
public class AlignedIsotopeGrouperTask extends AbstractTask {
	 ...
	 * @param parameters
	 */
	public AlignedIsotopeGrouperTask(MZmineProject project, PeakList peakList, ParameterSet parameters) {
		...
		suffix = parameters.getParameter(AlignedIsotopeGrouperParameters.suffix)
				.getValue();
		mzTolerance = parameters.getParameter(
				AlignedIsotopeGrouperParameters.mzTolerance).getValue();
		rtTolerance = parameters.getParameter(
				AlignedIsotopeGrouperParameters.rtTolerance).getValue();
		monotonicShape = parameters.getParameter(
				AlignedIsotopeGrouperParameters.monotonicShape).getValue();
		maximumCharge = parameters.getParameter(
				AlignedIsotopeGrouperParameters.maximumCharge).getValue();
		chooseMostIntense = (parameters.getParameter(
				AlignedIsotopeGrouperParameters.representativeIsotope).getValue() == AlignedIsotopeGrouperParameters.ChooseTopIntensity);
		removeOriginal = parameters.getParameter(
				AlignedIsotopeGrouperParameters.autoRemove).getValue();
		massList = parameters.getParameter(
				AlignedIsotopeGrouperParameters.massList).getValue(); 
		matchRaw = parameters.getParameter(AlignedIsotopeGrouperParameters.minMatchRawDP).getValue(); 
		minMatchRawDP = parameters.getParameter(AlignedIsotopeGrouperParameters.minMatchRawDP).getEmbeddedParameter().getValue(); 
	}
	...
	public void run() {
		...
		for (int r = 0; r < rows.length; r++) {
				...
				if(matchRaw) {
					...
					if(z>0) { 
						...
						if(bestRawIsoPattern!=null) {
							DataPoint[] isotopes = bestRawIsoPattern.toArray(new DataPoint[bestRawIsoPattern.size()]);  
							newPattern = new SimpleIsotopePattern(
									isotopes, IsotopePatternStatus.RAW, copy.toString()); 
						}
			...
			if (chooseMostIntense) {
				Arrays.sort(originalRows, new PeakListRowSorter(SortingProperty.Height, SortingDirection.Descending));
			}
			// selected row 
			PeakListRow selRow = originalRows[0];

			SimpleIsotopePattern newPattern = new SimpleIsotopePattern(
					isotopes, IsotopePatternStatus.DETECTED, selRow.toString());
		...
		logger.info(peakList.getNumberOfRows()+"\t\t"+deisotopedPeakList.getNumberOfRows()+"\t"
				+totalNoCharge+"\t\t"+rawFilledIn+"\t\t"+(foundIsotopeRow+rawFilledIn)+"\t\t"+foundIsotopeRow);
		...
		.addDescriptionOfAppliedTask(new SimplePeakListAppliedMethod(
				"Aligned isotopic peaks grouper", parameters));
	...
	/**
	 * try to find isotope peak in highest raw files
	 * highest data point +- FWHM/2
	 * @param rawDataFiles
	 * @param copy
	 * @return
	 */
	...
	/**
	 * find pattern in raw file with charge
	 * only keeps searching if the matched data points are greater than minMatchRawDP
	 * @param raw
	 * @param scanNumbers
	 * @param fittedDP
	 * @param dp
	 ...
	 * @return
	 */
	private int findRawPattern(RawDataFile raw, int[] scanNumbers, Vector<DataPoint> fittedDP, DataPoint dp, int charge) {
		...
		do {
			...
			dpMaxHeight = null;
			// mass 
			double isotopeMZ = dp.getMZ() + isotopeDistance * n / (double) charge;
			...
			for(int s=0; s<scanNumbers.length; s++) {
				...
				if(mass!=null) {
					...
					for(int i=0; i<allDP.length; i++) {
						// in mz tolerance?
						if ((!monotonicShape || allDP[i].getIntensity()<dp.getIntensity()) && mzTolerance.checkWithinTolerance(allDP[i].getMZ(), isotopeMZ)) { 
	...
	/**
	 * searches for the maximum row in distance
	 * avoiding bad mass detection and shoulder peaks
	 * @param rawFiles
	 * @param rows
	 * @param r
	 * @return
	 ...
	 * @return
	 */
	private int searchForMaxRowInDist(RawDataFile[] rawFiles, PeakListRow[] rows, int r, int maxRow, int detections, double maxH) {
		// escape
		...
		else return searchForMaxRowInDist(rawFiles, rows, r+1, maxRow, detections, maxH);
	}
	 ...
	 */
	private void fitPattern(RawDataFile[] rawFiles, Vector<PeakListRow> fittedRows, int rowI, int charge,
			PeakListRow[] sortedRows) {
		if (charge == 0)
			return;
	...
	/**
	 * adds the row if there is no other isotope peak (same m/z)
	 * or if the intensity is higher
	 * @param list
	 * @param row
	 * @return
	 */
	...
	private void findPattern(RawDataFile raw, Feature p, int charge, Vector<PeakListRow> tmpFitted,
		...
		do {
			...
			for (; ind < rows.length; ind++) {
				...
				double isotopeMZ = candidatePeakMZ - isotopeDistance * n / (double) charge;
	 ...
	 */
	public static int find13CIsotope(PeakList peakList, PeakListRow row1, PeakListRow row2, int maximumCharge, MZTolerance mzTolerance) {
