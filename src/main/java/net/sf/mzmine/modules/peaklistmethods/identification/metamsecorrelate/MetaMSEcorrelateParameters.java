...
public class MetaMSEcorrelateParameters extends SimpleParameterSet {
    ...
    public static final ComboParameter<Object> GROUPSPARAMETER = new ComboParameter<Object>(
            "Sample set", "Paremeter defining the sample set of each sample. (Set them in Project/Set sample parameters)",
            new Object[0]);

    // minimum of samples per set (with the feature detected or filled in (min height?)
    // ... showing RT<=tolerance and r>=MIN_PEARSON_R
    public static final PercentParameter MIN_SAMPLES = new PercentParameter(
            "Min samples",
    ...
    public static final DoubleParameter MAIN_PEAK_HEIGHT = new DoubleParameter(
            "Main peak height", "Starts with grouping all features >= mainPeakHeight", 
    ...
    public static final IntegerParameter MIN_DP_CORR_PEAK_SHAPE = new IntegerParameter(
            "Min data points", "Minimum of data points to be used for correlation of peak shapes.", 
            3,3,100000);
    ...
    // deisotoping: use groups, mztolerance, min height, check boxes for isotopes, or list like adducts? monotonic
    public static final DoubleParameter ISO_MIN_HEIGHT = new DoubleParameter("Min isotopic peak height",
            "Minimum of a peak height used to search isotopic peaks in the raw data.");
    public static final BooleanParameter ISO_MONOTONIC = new BooleanParameter("Monotonic 13C isotope pattern",
            "Search for a monotonic 13C isotope pattern. Still finds other specified isotopes.",
            true); 
    public static final BooleanParameter ISO_RAW_SEARCH = new BooleanParameter("Search isotopic peaks in raw data",
            "Search for a monotonic 13C isotope pattern. Still finds other specified isotopes.",
            true);
    // elements
    public static final BooleanParameter ISO_CL = new BooleanParameter("Cl",
            "Search for Cl", false);
    public static final BooleanParameter ISO_BR = new BooleanParameter("Br",
            "Search for Br", false);
    public static final BooleanParameter ISO_FE = new BooleanParameter("Fe",
            "Search for Fe", false);
    public static final BooleanParameter ISO_S = new BooleanParameter("S",
            "Search for S", false);
    ...
    public static final BooleanParameter SEARCH_ADDUCTS = new BooleanParameter("Search for adducts",
            "Search for adducts.", true);
    public static final BooleanParameter POSITIVE_MODE = new BooleanParameter("Positive MS mode",
            "Positive or negative mode?", false);
    public static final OptionalParameter<PercentParameter> ADDUCT_BONUSR = new OptionalParameter<>(
    		new PercentParameter("Bonus for adduct", 
    				"Bonus correlation r that is added to the peak shape correlation before checking with the minimal r.", 
    				0.10, 0, 1));
    		
    public static final IntegerParameter MAX_CHARGE = new IntegerParameter(
            "Maximum charge", "Maximum charge to be used for adduct search.", 
            3,1,100);
    public static final IntegerParameter MAX_MOLECULES = new IntegerParameter(
            "Maximum molecules/cluster", "Maximum molecules per cluster (f.e. [2M+Na]+).", 
            3,1,10);
    public static final IntegerParameter MAX_COMBINATION = new IntegerParameter(
            "Maximum combination", "Maximum combination of adducts (set in the list) (f.e. [M+H+Na]2+ = combination of two).", 
            3,1,10);
    public static final IntegerParameter MAX_MODS = new IntegerParameter(
            "Maximum modification", "Maximum modification of adducts (set in the list)", 
            3,1,10);
    
    public static final ESIAdductsParameter ADDUCTS = new ESIAdductsParameter(
            "Adducts", "List of adducts, each one refers a specific distance in m/z axis between related peaks");
    ...
    public ExitCode showSetupDialog(Window parent, boolean valueCheckRequired) {
        ...
        if(choices.length>1) getParameter(MetaMSEcorrelateParameters.GROUPSPARAMETER).setValue(choices[1]);
