package net.sf.mzmine.imzmltmp;
...
import com.alanmrace.jimzmlparser.imzML.ImzMLTest;
import com.alanmrace.jimzmlparser.imzml.ImzML;
import com.alanmrace.jimzmlparser.mzml.Spectrum;
import com.alanmrace.jimzmlparser.parser.ImzMLHandler;


public class ImzMLWindow {
...
//    public static final String TEST_RESOURCE = "/src/test/resources/MatrixTests_N2.imzML";
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ImzMLWindow window = new ImzMLWindow();
...
//            instance = ImzMLHandler.parseimzML("C:/DATA/MALDI Sh/von Rebeca/20170309_ProbeF_10min_Subli._R/AREA01/imzml/20170309_MADLI_III_20170309_ProbeF_10min_Subli-001.imzML");
	...
	private void loadBigData() {
        try {
            instance = ImzMLHandler.parseimzML("C:/DATA/MALDI Sh/examples/HR2MSI mouse urinary bladder S096.imzML");
		...
		pnImage = new JPanel() {
			...
			public void paint(Graphics g) {
				...
				if(z!=null)  {
					...
					for(int x = 0; x<z.length; x++) {
						for(int y = 0; y<z[x].length; y++) {
							Color c = interpolate(Color.BLUE, Color.RED, (float)((z[x][y]-zMin)/(zMax-zMin)));
							g.setColor(c);
	...
	public static File getPathOfJar() {
    	 ...
    	 */
    	try {
    	File jar = new File(ImzMLWindow.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath());
